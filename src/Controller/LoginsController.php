		...
		if ($this->request->is('post')) {
			...
			if ($user['role_id'] == 1) {
				...
				if ($this->request->data['remember_me'] == 1) {
					$expires = '+1 month'; // Set the cookie to expire in 1 month
					$this->Cookie->write('remember_me', $this->request->data['remember_me'], true, $expires);
					$this->Cookie->write('email', $this->request->data['email'], true, $expires);
					$this->Cookie->write('password', $this->request->data['password'], true, $expires);
				} else {
					// Delete the cookies by writing empty values with an immediate expiration
					$this->Cookie->write('remember_me', '', false);
					$this->Cookie->write('email', '', false);
					$this->Cookie->write('password', '', false);
				}
