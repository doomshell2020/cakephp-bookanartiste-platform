...
class PackageController extends AppController
{
	...
	$this->loadModel('Users');
	//pr($transcation); die;
	$profile_id = $transcation['profile_id'];
	$number_of_days = $transcation['number_of_days'];
	$currentdate=date('Y-m-d H:m:s');
	$expiredate=date('Y-m-d H:m:s',strtotime($currentdate."+$number_of_days days"));
	$profile = $this->Users->get($profile_id);
	$profile->featured_expiry = $expiredate;
    ...
    */
	...
	$this->loadModel('Banner');
		...
		if ($this->request->is(['post', 'put']))
			...
			$this->request->data['banner_date_from'] =date('Y-m-d',strtotime($this->request->data['banner_date_from']));
			$this->request->data['banner_date_to'] =date('Y-m-d',strtotime($this->request->data['banner_date_to']));
	...
	$banner_price = $this->request->data['banner_price'];
	$user_id = $this->request->session()->read('Auth.User.id');
	    $transcation_data['payment_method'] = 'Paypal';
	    $transcation_data['amount'] = $banner_price;
	    $transcation_data['user_id'] = $user_id;
