...
class ProfileController extends AppController{
	public function initialize()
	{
	    parent::initialize();
	    $this->
Auth->allow(['index', 'getStates', 'getcities', 'reportspam', 'reportprofile','viewprofile','viewvitalstatistics','viewprofessionalsummary','viewperformance','viewgalleries',
'viewimages','viewaudios','viewvideos','viewschedule','viewreviews']);
	}
	...
	$review = $this->Review->find('all')->contain(['Requirement'])->where(['Review.artist_id' => $current_user_id,'Review.id' => $job_review])->toarray();
...
public function imagecrop()
	...
	public function profilecounter()
	{
		...
		$count = $this->Profilecounter->find('all')->where(['Profilecounter.user_id' => $id,'Profilecounter.profile_id' => $this->request->data['data']])->first();
		if($count==0 && $id !=$this->request->data['data'])
		{
		$this->request->data['user_id']=$id;
...
public function changeProfilePic() {
	...
	$this->request->data['profile_image']=	 $profile['profile_image'];
		...
		pr($this->request->data['profile_image']);exit;
	...
	$performance_subgenre = $this->Performance_genre->find('all')->select(['id', 'subgenre_id'])->where(['Performance_genre.user_id' => $id, 'Performance_genre.genre_id' => 0])->order(['Performance_genre.id' => 'DESC'])->toarray();
	$this->set('performance_subgenre', $performance_subgenre);
	$payfrequency = $this->Performancedesc2->find('all')->contain(['Currency','Payment_fequency'])->where(['user_id' => $id])->toarray();
	//pr($payfrequency);die;
	$this->set('payfrequency', $payfrequency);
	$currency = $this->Currency->find('list')->select(['id', 'name', 'symbol'])->order(['Currency.id' => 'DESC'])->toarray();
	$this->set('currency', $currency);
	...
	$review = $this->Review->find('all')->contain(['Requirement'])->where(['Review.artist_id' => $current_user_id])->toarray();
	$this->set('review', $review);
