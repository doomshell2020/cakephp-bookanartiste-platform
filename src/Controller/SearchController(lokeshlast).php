...
if($jobidcount < $details['number_of_vacancy'] && count($profilecount) <= $details['number_of_vacancy'] ){
					...
					$refernamebookingalreadyask = $key;
					$invitedalbookingreadyask[$refernamebookingalreadyask] = $profilecountarrayss[$i];
					$session = $this->request->session();
					$session->write('bookingalreadyask',$invitedalbookingreadyask);
					$this->Flash->error(__('The following profiles have already been uploaded'), ['key' => 'alreadybooknow_job_fail']);
				}
	...
	unset($_SESSION['askquoteinvitealreadyask']);
		...
		$nontalentuser_id=$this->request->session()->read('Auth.User.id');
			...
			$askquoteavb= $this->askquotechecked($this->request->data['job_id'],$this->request->data['user_id']);
				...
				$x= 0; foreach($this->request->data['job_id'] as $key=>$value){ 
			if($this->request->data['job_id'][$key][0]!=''){		
		$profilecount	= explode(",", $this->request->data['user_id']);
			$jobidcountnew=$this->JobQuote->find('all')->where(['JobQuote.job_id'=>$key,'JobQuote.user_id'=>$profilecount[$x]])->count();
	...
	$i++; }
			...
			$jobidcount=$this->JobQuote->find('all')->where(['JobQuote.job_id'=>$key,'JobQuote.user_id'=>$profilecountarray[$i]])->count();
				...
				if($jobidcount<=0){
					...
					$invitedalreadyask[$refernamealreadyask] = $profilecountarray[$i];
					$session = $this->request->session();
					$session->write('askquoteinvitealreadyask',$invitedalreadyask);
					$this->Flash->error(__('The following profiles have already been uploaded'), ['key' => 'alreadyask_job_fail']);
					}
				}
					else{
