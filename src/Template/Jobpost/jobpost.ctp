...
<script>
  $(document).ready(function() {
    var jobIdString = '<?php echo $job_id; ?>';
    var isJobId = jobIdString ? parseInt(jobIdString, 10) : null;
    console.log('>>>>>>>', isJobId);
    ...
    <div class="container profile-bg requirement ">
      ...
      <div class="form-group">
        ...
        <div class="col-sm-8">
          ...
          <?php echo $this->Form->input('telent_type', array('class' => 'longinput form-control skillsdata', 'maxlength' => '20', 'placeholder' => 'Skills', 'label' => false, 'value' => implode(", ", $array1), 'id' => 'skillshow', 'required' => true)); ?>
          <input type="hidden" name="data[telent_type][]" id="skill" value="<?php echo  implode(",", $array); ?>" />
      ...
      <script>
        var max_days = parseInt('<?php echo $number_valid_of_days; ?>', 10);
        var totalEligibleSkills = parseInt('<?php echo $total_eligible_skills; ?>', 10);

        $("#skill_error").html(`Please select ${totalEligibleSkills}  talent type for this job post.`);
          ...
          $("input:checkbox[class=rooms]").each(function() {
            if ($(this).is(":checked")) {
              ...
              if (checkedSkillsCount > totalEligibleSkills) {
                var skillError = `Please select ${totalEligibleSkills}  talent for this job post. To increase this limit, please upgrade your Profile Package.`;
      ...
      <div class="form-group">
        ...
        <div class="col-sm-8">
          ...
          <!-- Radio Button for No (Default Selected) -->
          <label class="radio-inline">
            <input
              type="radio"
              name="continuejob"
              ...
              value="N"
              onclick="changerefertype(this)"
              <?php echo (!isset($requirement['continuejob']) || $requirement['continuejob'] == 'N') ? 'checked' : ''; ?>>
            No
          </label>
      ...
      <?php if (count($requirementvacancy) > 0) { ?>
        <span id="ncpy">
        <?php } else { ?>
          <span id="ncpy" style="display: none;">
          <?php } ?>
                    ...
                    <table class="table table-bordered">
                      <thead>
                        <tr>
                          ...
                          <th>Gender <span class="jobpostrequired">*</span>
                            <span class="pop-cnt p">
                              <a href="javascript:void(0)" data-toggle="popovessr" data-placement="bottom" data-content="<?php echo $genderdef; ?>"><i class="fa fa-question"></i>
                              </a>
                            </span>
                          </th>
                      ...
                      <tbody class="video_container">
                        <?php if (count($requirementvacancy) > 0) { ?>
                          ...
                          <?php
                          foreach ($requirementvacancy as $key => $value) {
                            // pr($requirementvacancy);
                            // die;
                            $skillname = $this->Comman->getSkillname($value['telent_type']);
                            $skillNameOptmise = str_replace(' ', '_', $skillname['name']);

                          ?>
                            <tr class="video_details" id="room_<?php echo $skillNameOptmise; ?>">

                              <td>
                                <?php echo $this->Form->input('requirementskills', array('class' => 'form-control', 'placeholder' => 'Requirement Vacancy', 'value' => $skillname['name'], 'required' => true, 'label' => false, 'type' => 'text', 'name' => 'data[requirementskills][]', 'readonly' => true)); ?>
                              </td>
                              ...
                              <?php
                              $gen = array('a' => 'Any', 'bmf' => 'Both Male And Female', 'm' => 'Male', 'f' => 'Female', 'o' => 'Other');
                              ?>
                              ...
                              <td>
                                ...
                                echo $this->Form->input('payment_freq', array(
                                  ...
                                  'options' => $payfreq,
                                  'name' => 'data[payment_freq][]',
                                  'value' => !empty($value['payment_freq']) ? $value['payment_freq'] : null // 👈 default to 10 if empty
                                ));
                                ?>
                              ...
                              <td>
                                <?php echo $this->Form->input('payment_currency', array('class' => 'form-control paycur', 'label' => false, 'empty' => '--Select Currency--', 'options' => $Currency, 'selected' => 'selected', 'name' => 'data[payment_currency][]', 'value' => $value['payment_currency'], 'title' => 'Please Select Payment Frequency (per) option')); ?>
                              </td>


                              <td>
                                <?php echo $this->Form->input('payment_amount', array('class' => 'form-control payamt', 'placeholder' => 'Open to Negotiations', 'id' => 'name', 'label' => false, 'type' => 'number', 'name' => 'data[payment_amount][]', 'value' => ($value['payment_amount'] > 0) ? $value['payment_amount'] : null, 'title' => 'Payment Amount')); ?>
                              </td>
                        ...
                        <tr class="video_details" id="room-1">
                          <td>
                            <?php echo $this->Form->input('requirementskills', array('class' => 'form-control', 'placeholder' => 'Requirement Vacancy', 'label' => false, 'type' => 'text', 'name' => 'data[requirementskills][]')); ?>
                          </td>
                          <td>
                            <?php echo $this->Form->input('number_of_vacancy', array('class' => 'form-control', 'placeholder' => 'Number of Vacancy', 'label' => false, 'type' => 'number', 'min' => 1, 'name' =>
                            'data[number_of_vacancy][]')); ?>
                          </td>
                          <?php $gen = array('a' =>
                          'Any', 'bmf' => 'Both Male And Female', 'm' => 'Male', 'f' => 'Female', 'o' => 'Other',); ?>
                          <td>
                            <?php echo $this->Form->input('sex', array('class' => 'form-control', 'placeholder' => 'Gender', 'label' => false, 'empty' => '--Select Gender--', 'options' => $gen, 'selected' => 'selected', 'name' =>
                            'data[sex][]')); ?>
                          </td>
                          ...
                          <td>
                            <?php echo $this->Form->input('payment_amount', array(
                              ...
                              'placeholder' => 'Open to Negotiations',
                              'label' => false,
                              'type' => 'number',
                              'min' => 1,
          ...
          <script>
            ...
            $(document).on('change', '.payfreq', function() {
              ...
              var $payamt = $row.find('.payamt');
              var $paycur = $row.find('.paycur');

              if (id) {
                $payamt
                  .removeAttr('disabled')
                  .removeAttr('title')
                  .attr('placeholder', 'Offer Amount')
                  .attr('required', true);
                ...
                $payamt
                  .prop('disabled', true)
                  .prop('title', 'Please Select Payment Frequency (per) option')
                  .attr('placeholder', 'Open to Negotiations')
                  .removeAttr('required');
              ...
              $("input:checkbox[class=chk]").each(function() {
                if ($(this).is(":checked")) {
                  ...
                  if (checkedskills > total_elegible_skills) {
                    skill_error = "Please select " + total_elegible_skills + "  talent for this job post";
            ...
            <div class="col-sm-10">
              <div class="row">
                <div class="col-sm-5 date">
                  ...
                  <?php
                  $eventDate = !empty($requirement['event_from_date']) ? date("F d,Y H:i", strtotime($requirement['event_from_date'])) : '';
                  echo $this->Form->input('event_from_date', array(
                ...
                <div class="col-sm-5 date">
                  <?php
                  //echo $this->Form->input('event_to_date', array('class' => 'form-control datetimepicker2', 'placeholder' => 'MM DD,YYYY', 'type' => 'text', 'required' => true, 'label' => false, 'value' => (!empty($requirement['event_from_date'])) ? '' : ''));
                  ?>

                  <?php
                  $eventToDate = !empty($requirement['event_to_date']) ? date("F d,Y H:i", strtotime($requirement['event_to_date'])) : '';

                  echo $this->Form->input('event_to_date', array(
            ...
            <div class="col-sm-10">
              <div class="row">
                <div class="col-sm-5 date">
                  ...
                  <?php
                  //echo $this->Form->input('last_date_app', array('class' => 'form-control datetimepicker3', 'readonly' => true, 'placeholder' => 'MM DD,YYYY', 'type' => 'text', 'id' => 'datepicker', 'required' => true, 'label' => false, 'value' => (!empty($requirement['last_date_app'])) ? '' : '')); 
                  ?>

                  <?php
                  // Format dates if they exist, otherwise set empty value
                  $lastDateApp = !empty($requirement['last_date_app']) ? date("F d,Y H:i", strtotime($requirement['last_date_app'])) : '';

                  // Last Date of Application
                  echo $this->Form->input('last_date_app', array(
            ...
            <div class="col-sm-10">
              <div class="row">
                <div class="col-sm-5 date">
                  ...
                  $talentRequiredFrom = !empty($requirement['talent_required_fromdate']) ? date("F d,Y H:i", strtotime($requirement['talent_required_fromdate'])) : '';
                  // Talent Required From Date
                  echo $this->Form->input('talent_required_fromdate', array(
                    'class' => 'form-control datetimepicker4',
                    'readonly' => true,
                    'placeholder' => 'MMMM DD, YYYY HH:mm',
                    'type' => 'text',
                    'id' => 'talentrequiredfrom',
                    'required' => true,
                    'label' => false,
                    'value' => $talentRequiredFrom
                  ));
                  ?>
                ...
                <div class="col-sm-5 date">
                  ...
                  $talentRequiredTo = !empty($requirement['talent_required_todate']) ? date("F d,Y H:i", strtotime($requirement['talent_required_todate'])) : '';

                  // Talent Required To Date
                  echo $this->Form->input('talent_required_todate', array(
                    'class' => 'form-control datetimepicker5',
                    'readonly' => true,
                    'placeholder' => 'MMMM DD, YYYY HH:mm',
                    'type' => 'text',
                    'id' => 'talentrequiredto',
                  ...
                  <script type="text/javascript">
                    ...
                    $(document).ready(function() {
                      ...
                      $('.datetimepicker3').datetimepicker({
                        ...
                        startDate: new Date(),
                        endDate: new Date(new Date().setDate(currentDate + max_days))
                      });

                      let dateEndate = $('.datetimepicker3').datetimepicker('getDate', max_days);
                      dateEndate.setDate(dateEndate.getDate() + max_days);
                      $('.datetimepicker3').datetimepicker('setDate', dateEndate);
                      var jobIdString = '<?php echo $job_id; ?>';
                      var isJobId = jobIdString ? parseInt(jobIdString, 10) : null;
                    ...
                    function changerefertype(values) {
                      ...
                      if (values.value == 'Y') {
                        $("#event-from-date").removeAttr("required"); // Remove required attribute
                        $("#event-to-date").removeAttr("required"); // Remove required attribute

                        // let future_date = new Date();
                        // let current_date = new Date();
                        // future_date.setDate(current_date.getDate() + max_days);

                        // $('#talentrequiredfrom, #talentrequiredto, #event-to-date, #event-from-date')
                        //   .attr('disabled', 'disabled')
                        //   .val('');

                        $(".talentreq, .talentevent").hide();
                        // $('#event-to-date').prop('title', 'Fill in Event From Date Time column');

                        // // Reset datetime picker values before setting new limits
                        // $('.datetimepicker3').val(""); // Clear input field
                        // $('.datetimepicker3').datetimepicker('setStartDate', current_date);
                        // $('.datetimepicker3').datetimepicker('setEndDate', null);

                        // // Set future max date
                        // future_date.setDate(future_date.getDate() + max_days);
                        // // console.log('>>>>> Future Date:', future_date);
                        // $('.datetimepicker3').datetimepicker('setEndDate', future_date);

                      } else {
                        // Enable input fields
                        $('#talentrequiredfrom, #talentrequiredto, #event-to-date, #event-from-date').removeAttr('disabled');
                        $(".talentreq, .talentevent").show();
                        $("#event-from-date").attr("required", "required"); // Add required attribute
                        $("#event-to-date").attr("required", "required"); // Add required attribute

                        // $('#event-to-date').removeAttr('title');

                        // Reset datetime picker and set a new start date
                        // $('.datetimepicker3').val(""); // Reset "From Date"
                        // $('.datetimepicker3').datetimepicker('setStartDate', new Date());
                      }
                    }

                    // Ensure the DateTimePicker closes after selecting a date
                    ...
                    $(function() {
                      let start = new Date();
                      let end = new Date(new Date().setFullYear(start.getFullYear() + 4));
                      ...
                      $('.datetimepicker1').datetimepicker({
                        ...
                        let toDate = new Date($('.datetimepicker2').val());
                        $('.datetimepicker2').prop('disabled', false);

                        if (toDate < fromDate) {
                          $('.datetimepicker2').val(""); // Reset "To Date"
                        }

                        $('.datetimepicker2').datetimepicker('setStartDate', fromDate);
                        $('.datetimepicker3').datetimepicker('setStartDate', fromDate);
                        $('.datetimepicker5').datetimepicker('setStartDate', fromDate);

                        document.getElementById("talentrequiredfrom").defaultValue = $(this).val();
                        $(this).datetimepicker('hide');
                      });
                      ...
                      $('.datetimepicker2').datetimepicker({
                        ...
                        let fromDate = new Date($('.datetimepicker1').val());

                        if (fromDate > toDate) {
                          $('.datetimepicker1').val(""); // Reset "From Date"
                        }

                        $('.datetimepicker1').datetimepicker('setEndDate', toDate);
                        $('.datetimepicker3').datetimepicker('setEndDate', toDate);
                        $('.datetimepicker4').datetimepicker('setEndDate', toDate);

                        // ✅ Check if chosen "To Date" is greater than (Current Date + max_days)
                        let maxAllowedDate = new Date(Date.now() + max_days * 24 * 60 * 60 * 1000);
                        // console.log('Max Allowed Date:', maxAllowedDate);
                        // console.log('Chosen To Date:', toDate);

                        if (toDate < maxAllowedDate) {
                          console.log('Updating datetimepicker3 as To Date exceeds max days.');
                          $('.datetimepicker3').datetimepicker('setDate', toDate);
                        } else {
                          console.log('To Date exceeds max days. Setting to max allowed date.');
                          $('.datetimepicker3').datetimepicker('setDate', maxAllowedDate);
                        }

                        document.getElementById("talentrequiredto").defaultValue = $(this).val();
                        $(this).datetimepicker('hide');
                      });

                      // Last Application Date Picker
                      $('.datetimepicker3').datetimepicker({
                        format: 'MM dd,yyyy hh:ii',
                        language: 'en',
                        pickTime: false,
                      ...
                      // Talent Required Date, Time From (datetimepicker4)
                      $('.datetimepicker4').datetimepicker({
                        ...
                        let talentRequiredFrom = new Date($(this).val());
                        let fromDate = new Date($('.datetimepicker1').val());
                        let toDate = new Date($('.datetimepicker2').val());

                        if (talentRequiredFrom < fromDate || talentRequiredFrom > toDate) {
                          alert("Talent Required Date From must be between the Event Start and End Date.");
                          $(this).val(""); // Reset input
                          return;
                        }

                        $('.datetimepicker5').datetimepicker('setStartDate', talentRequiredFrom);
                        $(this).datetimepicker('hide');
                      });

                      // Talent Required Date, Time To (datetimepicker5)
                      $('.datetimepicker5').datetimepicker({
                        ...
                        let talentRequiredTo = new Date($(this).val());
                        let talentRequiredFrom = new Date($('.datetimepicker4').val());

                        if (talentRequiredTo < talentRequiredFrom) {
                          alert("Talent Required Date To must be after Talent Required Date From.");
                          $(this).val(""); // Reset input
                          return;
                        }
                    ...
                    // $(function() {
                    //   let start = new Date();
                    //   let end = new Date(new Date().setYear(start.getFullYear() + 4));
                    ...
                    //     // Ensure Talent Required To (datetimepicker5) is within limits
                    //     //$('.datetimepicker5').datetimepicker('setEndDate', toDate);

                    //     let lastApplicationTo = $(this).val();
                    //     let lastApplicationDate = new Date($('.datetimepicker3').val());

                    //     // if (lastApplicationDate > toDate) {
                    //     //   $('.datetimepicker3').val(toDate.toISOString().slice(0, 16)); // Set to event "To Date"
                    //     // }
                    ...
                    //     $('.datetimepicker5').datetimepicker('setStartDate', talentRequiredFrom);
                    //     //$('.datetimepicker5').datetimepicker('setEndDate', toDate); // Ensure it doesn’t exceed event end date
                    //     $(this).datetimepicker('hide');
                    //   });
                    ...
                    //     let talentRequiredTo = new Date($(this).val());
                    //     let talentRequiredFrom = new Date($('.datetimepicker4').val());
                    //     let eventEndDate = new Date($('.datetimepicker2').val());

                    //     // Ensure Talent Required To is after Talent Required From
                    //     if (talentRequiredTo < talentRequiredFrom) {
                    //       alert("Talent Required Date To must be after Talent Required Date From.");
                    //       $(this).val(""); // Reset input
                    //       return;
                    ...
                    //     // Ensure Talent Required To is within event end date
                    //     // if (talentRequiredTo > eventEndDate) {
                    //     //   alert("Talent Required Date To cannot be after the Event End Date.");
                    //     //   $(this).val(""); // Reset input
                    //     //   return;
                    //     // }
  ...
  $(document).ready(function() {
    ...
    $('.rooms').change(function() {
      ...
      if (isChecked) {
        ...
        clone.find('#number-of-vacancy, #sex').attr('required', true);
