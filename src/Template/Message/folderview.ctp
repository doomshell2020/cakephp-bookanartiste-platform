...
<div class="mess-box-container">
      <div class="container">
       ...
       <div class="row profile-bg m-top-20">
        <h4 class="text-center"><?php echo $folder_info['folder_name']; ?></h4>
...
top: 89px;">
	...
	<input type="hidden" name="message_id" id="message_id" value="<?php echo ($messages[0]['id'] > 0)?$messages[0]['id']:'0'; ?>">
	 <input type="hidden" name="type" id="type" value="<?php if($messages[0]['from_id']==$this->request->session()->read('Auth.User.id') && $messages[0]['from_box']!='trash'){ echo "s"; }else{ if($messages[0]['from_box']!='trash'){ echo "i"; }else{ echo 't'; }} ?>">
    </div> 
		...
		foreach($messages as $messages_data){  // pr($messages_data);
		    ...
		    <div class="col-sm-2"><img src="<?php echo SITE_URL; ?>/profileimages/<?php echo $messages_data['from_image']; ?>" class=" img-circle pull-left"><p class="pull-right">
		    ...
		    <?php echo $messages_data['subject']; ?> <?php if($messages_data['total'] > 1){ echo "(".$messages_data['total'].")"; }?></a></p></div>
		    <div class="col-sm-3 time"><p><?php echo date("d-m-y h:i",strtotime($messages_data['created'])); ?></p></div>
		    </div>
	    <?php }else{?>
		  <div class="row inbox-row box" id="row_<?php echo $messages_data['id']; ?>">
             <div class="col-sm-1"><input type="checkbox" name="thread_id[]" class="thread_id" id="thread_id" value="<?php echo $messages_data['thread_id']; ?>"></div>
             <div class="col-sm-2"><img src="<?php echo SITE_URL; ?>/profileimages/<?php echo $messages_data['to_image']; ?>" class=" img-circle pull-left"><p class="pull-right">
             <?php if($messages_data['total'] > 1){ ?>
             <a href="<?php echo SITE_URL; ?>/message/viewmessage/<?php echo $messages_data['thread_id']; ?>">
             <?php }else{?>
		<a href="<?php echo SITE_URL; ?>/message/view/<?php echo $messages_data['id']; ?>">
		...
		<strong><?php echo $messages_data['to_name']; ?></strong>
             ...
             <?php }?>
             <?php if($messages_data['read_status']=='Y'){ ?>
             <?php echo $messages_data['subject']; ?> <?php if($messages_data['total'] > 1){ echo "(".$messages_data['total'].")"; }?>
             <?php }else{?>
             <strong><?php echo $messages_data['subject']; ?> <?php if($messages_data['total'] > 1){ echo "(".$messages_data['total'].")"; }?></strong>
             <?php }?>
