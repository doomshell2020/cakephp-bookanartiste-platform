...
<?php //pr($bannerrec); ?>
 ...
 <div>
   <?php echo $bannerrec['title']." Homepage Banner"; ?>
 </div>
  <div>
   <label><strong>Banner URL</strong></label>
 </div>
 <div>
   <?php echo $bannerrec['bannerurl']; ?>
 </div>
  <div>
   <label><strong>Banner Amount</strong></label>
 </div>
 <div>
   $<?php echo $bannerrec['amount']; ?>
 </div>
</div>

 <div class="col-sm-6">
    <div>
     <h5>Total Amount Before Tax: <span><?php echo "$".$bannerrec['amount']; ?> </span></h5>
  </div>
  ...
  <div class="">
     <h5>Total Bill Amount: <span><?php $grandtotal=$bannerrec['amount']+($bannerrec['amount']*$totaltax/100); echo "$".$grandtotal; ?></span></h5>
   </div>
...
<?php echo $this->Form->create('Package',array('url' => array('controller' => 'Package', 'action' => 'bannerprocessrepayment'),'type'=>'file','inputDefaults'=>array('div'=>false,'label'=>false),'class'=>'form-horizontal','id'=>'PackageIndexForm','autocomplete'=>'off')); ?>
  	
  	<input type="hidden" name="banner_id" value="<?php echo $bannerrec['id']; ?>">
    <input type="hidden" name="before_tax_amt" value="<?php echo  $bannerrec['amount']; ?>">
