...
<!-----------------------editprofile-strt------------------------>
 ...
 <?php if ($package_type=='AC') {
   $package_t="Ask For Quote";
 } ?>
  <div class="row">
      <div class="col-sm-6">
      <div>
	...
	<h5>Package Amount</h5>
       ...
       <div>
	$<?php echo $pcakgeinformation['total_price']; ?>
    </div>
  </div>

    <div class="col-sm-6">
  <div>
   <h5>Total Amount Before Tax: <span><?php echo "$".$pcakgeinformation['total_price']; ?> </span></h5>
</div>
...
<div class="">
   <h5>Total Bill Amount: <span><?php $grandtotal=$pcakgeinformation['total_price']+($pcakgeinformation['total_price']*$totaltax/100); echo "$".$grandtotal; ?></span></h5>
 </div>
...
<?php echo $this->Form->create('Package',array('url' => array('controller' => 'Package', 'action' => 'buyquoteprocessrepayment'),'type'=>'file','inputDefaults'=>array('div'=>false,'label'=>false),'class'=>'form-horizontal','id'=>'PackageIndexForm','autocomplete'=>'off')); ?>
    ...
    <input type="hidden" name="package_price" value="<?php echo $grandtotal; ?>">
  	<input type="hidden" name="before_tax_amt" value="<?php echo $pcakgeinformation['total_price']; ?>">
 <?php if ($invoicereceipt) {
