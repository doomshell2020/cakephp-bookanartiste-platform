...
foreach ($friends as $friendss) { //pr($ethnicityfetch);
  ...
  <div class="col-sm-12 prsnl-det">
    ...
    <div class="row">
      <div class="col-sm-12">
        ...
        <input type="hidden" id="phonecode" class="form-control" name="tab" value="contacts">
    ...
    <div class="talent_tab">
      ...
      <ul class="nav nav-pills">
        ...
        <li class="active"><a data-toggle="" href="<?php echo SITE_URL ?>/profile/allcontacts/<?php echo $profile['user']['id']; ?>" class="cnt" data-action="contacts">Contacts (<?php echo count($friends); ?>)</a></li>
        <?php
          ...
          <ul class="dropdown-menu contact_all_refine">
            <li>
              <div class="col-sm-12">
                ...
                <!-- <script>
    $(document).ready(function() {
        // Initialize the slider range with the specified range and initial values
        $("#slider-range").slider({
            range: true, // Enable range selection
            min: <?php echo $minyears; ?>, // Set the minimum age based on PHP calculation
            max: <?php echo $maxyears; ?>, // Set the maximum age based on PHP calculation
            values: [18, 54], // Set the initial values of the range to 18 and 54
            slide: function(event, ui) {
                // Update the displayed range values as the slider is moved
                $("#amount").val(ui.values[0] + " - " + ui.values[1]);
                // Update the hidden input with the selected range values
                $("#sliderrangevalueset").val(ui.values[0] + " - " + ui.values[1]);
            }
        });

        // Set the initial value of the displayed range to match the initial slider values
        $("#amount").val("18 - 54");
    });
</script> -->
            ...
            <div class="row">
              <div class="prosrrc">
                <?php foreach ($friends as $friendss) {   ?>
                  ...
                  <script>
                    $(document).ready(function() {
                      $(".profileshare<?php echo $friendss['user_id']; ?>").click(function() {
                        $(".profileshare-toggle<?php echo $friendss['user_id']; ?>").slideToggle();
                      });
                    });
                  </script>
                  ...
                  <div class="col-sm-3 ">
                    ...
                    <div class="profile-det-img">
                      <div class="hvr-icon"><input type="checkbox" name="profile[]" class="profilesavecontact">
                        <a href="#" class="fa fa-remove"></a>
                      </div>
                      <a style="Width:100%;" target="_blank" href="<?php echo SITE_URL; ?>/viewprofile/<?php echo $friendss['user_id']; ?> ">
                        <img src="<?php echo SITE_URL; ?>/profileimages/<?php echo $friendss['profile_image'];  ?>" onerror="this.onerror=null;this.src='<?php echo SITE_URL ?>/images/noimage.jpg';">
                      </a>
                      <div class="img-top-bar">
                        ...
                        <?php $likesuser = $this->Comman->likess($friendss['user_id']); //pr($likesuser); 
                        ?>
                        <?php if ($subrepa) { ?>
                    ...
                    <div class="allcontact-social">
                      ...
                      <?php $contactrequest = $this->Comman->contactreqstatus($friendss['user_id']); ?>
                      ...
                      <?php } elseif ($contactrequest == 'S') { ?>
                        <a style="color: yellow" id="add_friend<?php echo $friendss['user_id']; ?>" class="managefriends fa fa-handshake-o" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true" href="javascript:void(0)" data-val="<?php echo $friendss['user_id']; ?>" data-action="cancel" title="Cancel Request"></a>

                      <?php } elseif ($contactrequest == 'C') { ?>
                        <a id="add_friend<?php echo $friendss['user_id']; ?>" class="managefriends fa fa-handshake-o" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true" href="javascript:void(0)" data-val="<?php echo $friendss['user_id']; ?>" data-action="remove" title="Cancel Request"></a>

                      <?php } else { ?>
                        <a id="add_friend<?php echo $friendss['user_id']; ?>" class="dropdown-toggle managefriends fa fa-user-plus" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true" href="javascript:void(0)" data-val="<?php echo $friendss['user_id']; ?>" data-action="connect"> </a>
                      <?php
                      } ?>

                      <a href="<?php echo SITE_URL; ?>/message/sendmessage/<?php echo ($friendss['user_id']) ? $friendss['user_id'] : '0' ?>" data-val="<?php echo ($friendss['user_id']) ? $friendss['user_id'] : '0' ?>" class="sendmessage" data-toggle="modal" data-target="#sendmessage" title="Send Message"><i class="fa fa-envelope"></i></a>
                      <!-- <a href="#" class="fa fa-user"></a> -->
                      <!-- Button trigger modal -->

                      <a data-toggle="tooltip" href="javascript:void(0)" class="profileshare<?php echo $friendss['user_id']; ?>" data-toggle="tooltip" title="Share"><i class="fa fa-share"></i></a>

                      <div class="share_button profileshare-toggle<?php echo $friendss['user_id']; ?>" style="display: none;">
                      ...
                      <a href="javascript:void(0)" class="likesactive <?php echo (isset($likesuser) && $likesuser > 0) ? 'active' : ''; ?>" id="likeprofile<?php echo $friendss['user_id']; ?>" data-toggle="tooltip" data-val="<?php echo ($friendss['user_id']) ? $friendss['user_id'] : '0' ?>" title="Like"><i class="fa fa-thumbs-up"></i></a>
                    ...
                    <div class="all-cnt-det">
                      <h5 class="text-center"><a href="<?php echo SITE_URL; ?>/viewprofile/<?php echo $friendss['user_id']; ?> "><?php echo $friendss['name']; ?></a></h5>
                      ...
                      <p class="text-center"> <a style="Width:100%;" target="_blank" href="<?php echo SITE_URL; ?>/viewprofile/<?php echo $friendss['user_id']; ?> "><?php echo $friendss['location']; ?></a></p>
                    </div>



                    <div class="btn-book text-center">
                      <a href="<?php echo SITE_URL; ?>/viewprofile/<?php echo $friendss['user_id'];  ?>" class="btn btn-default" style="font-size= 10px;">Book Now </a>
                      <a href="<?php echo SITE_URL; ?>/viewprofile/<?php echo $friendss['user_id'];  ?>" class="btn btn-primary" style="font-size = 10px;">Ask For Quote</a>
                    </div>
                    </div>
                  </div>
                <?php } ?>
