...
<?php foreach($mutualfrnd as $friendss){ //pr($ethnicityfetch);
...
function opentab()
 ...
 <?php  echo $this->Form->create('',array('url'=>array('controller'=>'profile','action'=>'search'),'type'=>'file','inputDefaults'=>array('div'=>false,'label'=>false),'id'=>'searchForm','class'=>'form-horizontal')); ?>
   ...
   <input type="text" class="form-control  chekallser" name="name" placeholder="search...">
		<input type="hidden" id="phonecode" class="form-control" name="tab" value ="mutual">
...
$(document).ready(function(){  //alert();
    ...
    <?php //////////////-mutuaal------------?>
    <div id="Mutual" class="tab-pane fade in active">
      <div class="col-sm-12">
     <div class="row">
		 	 <div class="prosrrc">
		   <?php  foreach($mutualfrnd as $mutualfrndss){  // pr($mutualfrndss); ?>
  ...
  <img src="<?php echo SITE_URL; ?>/profileimages/<?php echo $mutualfrndss['profile_image'];  ?>" onerror="this.onerror=null;this.src='<?php echo SITE_URL ?>/images/job.jpg';">
  </a>
     <div class="img-top-bar"> 
     <?php  $subprpa = $this->Comman->subscriprpack($mutualfrndss['from_id']); ?>
    <?php if ($subprpa){ ?>
    <a href="<?php echo SITE_URL; ?>/profilepackage" title="Profile package"><img src="<?php echo SITE_URL; ?>/images/profile-package.png"></a>
    <?php } ?>
     <?php  $subrepa = $this->Comman->subscrirepack($mutualfrndss['from_id']); ?>
<?php if ($subrepa){ ?>
   ...
   <div class="all-cnt-det"><h5 class="text-center"><a target="_blank" href="<?php echo SITE_URL; ?>/viewprofile/<?php echo $mutualfrndss['user_id']; ?> "><?php echo $mutualfrndss['name']; ?></a></h5>
     <?php $skills= $this->Comman->userskills($mutualfrndss['user_id']); ?>
   <p class="text-center"><a style="color:black;" target="_blank" href="<?php echo SITE_URL; ?>/viewprofile/<?php echo $friendss['user_id']; ?> "><?php if($skills)
     ...
     <p class="text-center"><a style="color:black;" target="_blank" href="<?php echo SITE_URL; ?>/viewprofile/<?php echo $friendss['user_id']; ?> "><?php echo $mutualfrndss['location']; ?></a></p></div>
     
     <div class="btn-book text-center">
     <a target="_blank" href="<?php echo SITE_URL; ?>/viewprofile/<?php echo $mutualfrndss['user_id']; ?>" class="btn btn-default">Book</a>
      <a target="_blank" href="<?php echo SITE_URL; ?>/viewprofile/<?php echo $mutualfrndss['user_id']; ?>" class="btn btn-primary">Ask For Quote</a>
     </div>
     </div>
       <?php } ?>
